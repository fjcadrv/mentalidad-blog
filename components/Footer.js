import { SocialFollowButtons } from './SocialButtons';
  ...
  <svg xmlns="http://www.w3.org/2000/svg" width="25" height="24" fill="none" viewBox="0 0 25 24" className="dark:opacity-50">
    ...
    <defs>
      <clipPath id="clip0_192_823">
        <path className="text-white fill-current" d="M0 0H24V24H0z" transform="translate(.5)"></path>
      </clipPath>
    </defs>
  </svg>
);
...
export default function Footer({ copyrightText, social, showSocialFollow = false }) {
  return (
    <footer className="flex flex-col items-center py-16 gap-4">
      {showSocialFollow && social && (
        <div className="flex flex-col items-center gap-3">
          <p className="text-xs font-bold uppercase opacity-60 dark:text-white">
            Síguenos en redes
          </p>
          <SocialFollowButtons social={social} />
        </div>
      )}
      <p className="mt-2 font-bold uppercase dark:text-white opacity-60 text-center text-sm px-4">
        {copyrightText}
      </p>
      <ThemeSwitcher />
    </footer>
